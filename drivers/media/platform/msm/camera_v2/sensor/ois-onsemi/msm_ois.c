...
uint32_t current_fw, uint8_t vcm)
{
	...
	rc = onsemi_read_dword(o_ctrl,0x8008,&act_ver);

	if(((current_fw >>24) == 0x05 && current_fw < Primax_VERNUM) ||
		((current_fw >>24) == 0x01 && current_fw < SEMCO_VERNUM))
			update_fw = true;
	...
	if( update_fw )
	{
		...
		if((current_fw >>24) == 0x05 && vcm == 0x00)
		{
			pr_info("%s:PRIMAX vcm version 1\n", __func__);
			rc = F40_FlashDownload(o_ctrl, (uint8_t)mode, (current_fw>>24), 0x00);
		}
		else if((current_fw >>24) == 0x05 && vcm == 0x02)
		{
			pr_info("%s:PRIMAX vcm version 2\n", __func__);
			rc = F40_FlashDownload(o_ctrl, (uint8_t)mode, (current_fw>>24), 0x01);
		}
		else if((current_fw >>24) == 0x01)
		{
			pr_info("%s:SEMCO vcm\n", __func__);
			rc = F40_FlashDownload(o_ctrl, (uint8_t)mode, (current_fw>>24), ((act_ver & 0x0000FF00)>>8));
		}
...
static int32_t msm_ois_check_id(struct msm_ois_ctrl_t *o_ctrl) {
	...
	if((fw_version >>24) == 0x05)
	{
		strncpy(o_ctrl->vendor, "PRIMAX", 6);
		pr_info("%s: Update OIS vendor to PRIMAX\n", __func__);
	}
	else if((fw_version >> 24) == 0x01)
	{
		strncpy(o_ctrl->vendor, "SEMCO", 5);
		pr_info("%s: Update OIS vendor to SEMCO\n", __func__);
	}
	//ASUS_BSP Lucien ---: Add OIS SEMCO module
OIS_CHECK_FAIL:
    pr_err("%s: rc = %d X\n", __func__, rc);
